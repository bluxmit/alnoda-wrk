...
def get_env_vars_widget():
    ...
    var_select = ttk.TTkComboBox(list=state['envars'], pos=(r,row), size=(rs,1))
    ...
    def _saveBtn(): 
        ...
        if name == "": 
            ...
            return
        if value == "": 
            msg_lab._color = ERROR_COLOR; msg_lab._text = "Please value for the variable"; msg_lab.update()
            return
        # Start app/service
        ...
        msg_lab._color = WAIT_COLOR; msg_lab._text = "Variable added. Only in terminal." 
        msg_lab.update()
        return
    # Bind save button
